...
import { Search, User, ShoppingBag, ChevronDown, Menu, X, LogOut, Settings, LayoutDashboard, Loader2 } from 'lucide-react'
...
export default function Navbar() {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.length > 1) {
        setIsSearching(true)
        try {
          const res = await axios.get(`http://localhost:5001/api/products?q=${searchQuery}`)
  ...
  const handleLogout = () => {
    logout(); setShowUserMenu(false); setUserRole(null); navigate('/')
  }
    ...
    <>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 md:px-8 h-20 flex items-center justify-between relative">
          ...
          <div className={`flex items-center gap-4 md:gap-6 transition-all duration-300 ${isSearchOpen ? 'w-0 opacity-0 overflow-hidden md:w-auto md:opacity-100' : 'w-auto opacity-100'}`}>
            ...
            <div className="hidden lg:flex items-center gap-10 ml-8">
              {navLinks.map((link) => (
                <Link key={link.name} to={link.to} className="text-[14px] font-semibold text-slate-600 hover:text-black transition-all uppercase tracking-widest">{link.name}</Link>
              ))}
            </div>
          </div>

          {/* RIGHT SECTION: Search, User, Bag */}
                  ...
                  <div className="absolute right-0 mt-3 w-56 bg-white border border-slate-100 shadow-2xl rounded-sm py-3 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    {!user ? (
                        <div className="px-2 space-y-1">
                            <Link to="/login" onClick={() => setShowUserMenu(false)} className="block px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">Sign In</Link>
                            <Link to="/register" onClick={() => setShowUserMenu(false)} className="block px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">Sign Up</Link>
                        </div>
                    ) : (
                        <div className="px-2">
                            ...
                            {userRole === 'admin' && <Link to="/dashboard" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50"><LayoutDashboard size={16} /> Dashboard</Link>}
                            <Link to="/settings" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50"><Settings size={16} /> Settings</Link>
              ...
              <div className="absolute top-20 right-0 md:right-8 w-full md:w-[400px] bg-white border border-slate-100 shadow-2xl rounded-sm p-4 animate-in fade-in slide-in-from-top-2 z-[1001]">
                ...
                <div className="max-h-80 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                  {searchResults.map(item => (
                    <Link key={item._id} to={`/product/${item._id}`} onClick={() => {setIsSearchOpen(false); setSearchQuery('')}} className="flex items-center gap-4 group text-left">
                      <div className="w-12 h-14 bg-slate-50 overflow-hidden rounded-sm flex-shrink-0">
                        <img src={item.mainImage} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                      </div>
      ...
      <div className={`fixed inset-0 z-[9999] transition-all duration-500 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        ...
        <div className={`absolute left-0 top-0 bg-white w-[280px] h-full shadow-2xl p-8 transition-transform duration-500 ease-out flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <button className="absolute top-6 right-6 p-2 hover:bg-slate-100 rounded-full transition-colors" onClick={() => setIsOpen(false)}>
          ...
          <div className="flex flex-col gap-6 text-left">
            ...
            <Link to="/settings" onClick={() => setIsOpen(false)} className="text-xl font-bold text-slate-600 border-b border-slate-50 pb-3">
              Settings
            </Link>
          </div>
